...
class FFTConv(nn.Module):
    ...
    def __init__(
        ...
        **kernel_args,  # Arguments passed into inner convolution kernel
        ...
        self.F = nn.Parameter(torch.randn(channels, self.d_model))
    ...
    def forward(self, x, x_mask,state=None, rate=1.0, **kwargs): # absorbs return_output and transformer src mask
        """
        x: (B D L) if self.transposed else (B L D)
        """
        # print("FFTV_TRANS",self.transposed)
        ...
        # print("FFTV_4,",l_kernel,self.kernel1.d_model)
        k, k_state =  self.kernel1(L=l_kernel, rate=rate, state=state) # (C H L) (B C H L)
        # print("FFT_l_kernel1,",l_kernel,l_kernel+L)
        # print("FFTV_0",k.shape)
        ...
        # print("*******************", x.shape,x_f.shape,l_kernel+L)
        y_f = contract('bhl,chl->bchl', x_f, k_f)
        y = torch.fft.irfft(y_f, n=l_kernel+L)[..., :L]
        # Compute D term in state space equation - essentially a skip connection
        y = y + contract('bhl,ch->bchl', x, self.D)

        # print("FFT_X_MASK",x_mask.shape)
        if x_mask is not None:
            k_m, k_m_state =  self.kernel2(L=l_kernel, rate=rate, state=state) # (C H L) (B C H L)
            # print("FFTV_9,",k_m.shape)
            k_m = self.drop_kernel(k_m)
            # print("FFTV_10,",k_m.shape)
            # print("FFTV_8,",l_kernel,self.kernel2.d_model)
            k_m_f = torch.fft.rfft(k_m, n=l_kernel+L) # (C H L)
            # print("FFT_l_kernel2,",l_kernel,l_kernel+L)
            x_m_f = torch.fft.rfft(x_mask, n=l_kernel+L) # (B H L)
            # print("FFT_6: ", x_m_f.shape,k_m_f.shape)
            # print("***************",x_mask.shape,x_m_f.shape,l_kernel+L)
            y_m_f = contract('bhl,chl->bchl', x_m_f, k_m_f)
            y_m = torch.fft.irfft(y_m_f, n=l_kernel+L)[..., :L]
            y = y + y_m + contract('bhl,ch->bchl', x, self.F)
         # (B C H L)

        # Compute state update
        if state is not None:
            assert not self.bidirectional, "Bidirectional not supported with state forwarding"
            y = y + k_state+k_m_state #
            next_state = self.kernel.forward_state(x, x_mask,state)
        else:
            next_state = None
...
class S4Block(nn.Module):
    ...
    def forward(self, x, x_mask, lengths=None, **kwargs): # absorbs return_output and transformer src mask
        """
        ...
        if self.transposed: 
            ...
            x_mask = rearrange(x_mask, 'b d ... -> b ... d')
        L = x.size(1)
        ...
        # print("S4Block_1",x.shape)
        # print("S4Block_2",x_mask.shape)

        y, state = self.layer(x,x_mask,**kwargs)
